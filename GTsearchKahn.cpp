//in Kahn's algo, we make an in degree list,
//->if indeg(V) is 0, then all elements pointing to it are added to queue
...
public:
    ...
    vector<int> topologicalSort(int v=size)
    {
        // declaring ans to return
        vector<int> ans;
        find_TopoSort_BFS(adjList, ans, v);
        return ans;
    }
};
int main()
{
   ...
   cout << "Give the map:";
    ...
    G1.display_graph();
    vector<int> TS=G1.topologicalSort();
    for(auto i:TS){
        cout<<i<<" ";
    }
}
